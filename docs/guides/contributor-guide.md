...
| `persistence-jpa/` | `casehub-desiredstate-persistence-jpa` | `io.casehub.desiredstate.persistence.jpa` | `JpaFaultCountStore` -- JPA-backed `FaultCountStore` with Flyway migration V1. `FaultCountEntity` with composite key `(namespace, tenancy_id, node_id)`. `JpaReconciliationStateStore` -- JPA-backed `ReconciliationStateStore` with Flyway migration V2. Stores serialized `DesiredStateGraph` as JSON per tenant for orphan spec resolution across restarts. `ReconciliationStateEntity` with `tenancy_id` PK. `GraphSerializer` (package-private) handles Jackson ser/deser with FQCN discriminator for polymorphic `NodeSpec`. Portable SQL (H2 MODE=PostgreSQL + PostgreSQL). Tier 2 in CDI priority ladder -- classpath-activated, displaces `@DefaultBean` fallbacks. |
...
| `plugin/api/` | `casehub-desiredstate-plugin-api` | `io.casehub.desiredstate.plugin.api` | Step pipeline SPI: `StepPrimitive`, `StepResult`, `StepContext`, `StepParameters`, `YamlNodeSpec` (NodeSpec adapter for YAML-declared types), `PluginInterpolator` (prefix-based `${spec.*}`, `${auth.*}`, `${result.*}`, `${param.*}` interpolation), `ExpressionEvaluator` (recursive descent condition parser). Pure Java. |
| `plugin/runtime/` | `casehub-desiredstate-plugin` | `io.casehub.desiredstate.plugin.runtime` | Plugin YAML model (`PluginModel`, `PluginParser`, `PluginStepDef`, `PluginHeader`, `PluginSpecSchema`, `PluginFieldDef`, `CompoundPrimitiveDef`), `StepPipelineExecutor` (sequential execution with when/retry/skip), `PrimitiveRegistry`, built-in primitives (`CompareStatePrimitive`, `AssertPrimitive`, `JsonExtractPrimitive`, `RestCallPrimitive`), `YamlPluginProvisioner` (implements `NodeProvisioner`), `YamlPluginActualStateAdapter` (implements `ActualStateAdapter`), `CompoundPrimitiveExpander` (YAML-over-YAML-over-Java macro expansion), `YamlPluginRasRegistrar` (implements `SituationDefinitionProvider`), `CbrPluginMetadata`, `PluginDescriptor`. |
| `plugin/deployment/` | `casehub-desiredstate-plugin-deployment` | `io.casehub.desiredstate.plugin.deployment` | Quarkus build extension: `YamlPluginProcessor` scans `META-INF/desiredstate/plugins/*.yaml`, validates spec schema (field types, constraints), primitive existence (with Levenshtein typo suggestions), interpolation references (`${spec.*}` against schema, `${result.*}` against prior bindings), type conflicts with `@NodeTypeId` registry, `compare-state`/`approval-gate` constraints. Produces `PluginBuildItem`. |
...
| `DefaultReconciliationStateStore` | runtime | Wraps `InMemoryReconciliationStateStore` | `JpaReconciliationStateStore` (persistence-jpa) or custom store |
