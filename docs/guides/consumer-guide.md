...
| `annotations/runtime/` | `casehub-desiredstate-annotations` | Annotation-driven graph declarations. `@DesiredState`, `@Node`, `@DependsOn`, `@FaultPolicyDef`, `@GoalMethod`. The build extension (deployment module, auto-activated) scans annotations and generates `GoalCompiler` + `ThresholdFaultPolicy` CDI beans. |
