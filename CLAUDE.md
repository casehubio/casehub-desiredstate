...
| `plugin/runtime/` | `casehub-desiredstate-plugin` | `io.casehub.desiredstate.plugin.runtime` | `StepPipelineExecutor`, `YamlPluginProvisioner`, `YamlPluginActualStateAdapter`, built-in Java primitives (rest-call, graphql-call, json-extract, compare-state, assert, approval-gate), `YamlPluginCbrRegistrar`, `YamlPluginRasRegistrar`. Quarkus library. |
