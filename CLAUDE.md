...
|--------|----------|-------------|---------|
| `api/` | `casehub-desiredstate-api` | `io.casehub.desiredstate.api` | Core SPIs + domain types. Pure Java, Mutiny provided, CDI annotations provided. `@NodeTypeId` — type annotation for YAML-addressable NodeSpec classes (maps type string for registry discovery). |
...
| `annotations/runtime/` | `casehub-desiredstate-annotations` | `io.casehub.desiredstate.annotations` | Annotation-driven graph declarations: `@DesiredState`, `@Node`, `@DeclareNode`, `@DependsOn`, `@FaultPolicyDef`, `@Tier`, `@Customize`, `@DesiredStateQualifier`. Graph rewriting: `@GraphRule`, `@Match`, `@DirectDep`, `@Reaches`, `@NotExists`, `Direction` enum. Graph validation: `@GraphInvariant`. Two models: interface (`@DesiredState` + `@Node`) and class-based (`@DeclareNode`). `@DependsOn` supports string IDs and type-safe `Class<? extends NodeSpec>[]` refs. **Graph abstraction layer** (`graph` subpackage, future graph-core): `GraphView<N>`, `MutableGraphView<N>` (read/write view interfaces), `GraphReader<G,N>`, `GraphWriter<G,N>` (adapter interfaces), `GraphCycleException`. `DesiredStateGraphAdapter` (implements reader+writer for `DesiredStateGraph`/`DesiredNode`), `DesiredStateGraphView` (implements `MutableGraphView<DesiredNode>`). **Generic engines** — zero domain-specific imports (`DesiredNode`, `DesiredStateGraph`, `NodeId`, `NodeType`, `Dependency`); remaining api imports (`GraphMutation`, `ConflictingMutationException`) are generic types that move to graph-core at extraction (platform#267): `GraphRuleEngine` — `<N> MutableGraphView<N> evaluate(MutableGraphView<N>, List<ResolvedRule<N>>)`, fixed-point loop with pattern matching, conflict detection, cycle detection, mutation ordering. `GraphInvariantEngine` — `<N> void validate(GraphView<N>, List<ResolvedInvariant<N>>)`, single-pass validation with universal quantification. `ResolvedRule<N>` — `ImperativeRule<N>` (function closure), `ParameterizedRule<N>` (reflective), `DeclarativeRule<N>` (YAML/programmatic). `ResolvedInvariant<N>` — `ImperativeInvariant<N>` (consumer closure), `ParameterizedReflectiveInvariant<N>`, `DeclarativeInvariant<N>`. `PatternMatchingSupport` — generic `<N>` stateless matching primitives via `GraphView<N>`. `PatternEvaluator` — generic `<N>` pattern evaluation via `GraphView<N>`. `GraphPatternMatcher` — include/exclude graph matching with `!` prefix (`.gitignore`-style ordered evaluation). `@GraphRule.graph()` is `String[]` supporting exact, namespace wildcard (`pipeline:*`), global wildcard (`*:*`), and `!`-prefixed exclusions for standalone class discovery. `@Tier(nodeType)` optional attribute bypasses runtime `ReviewSpecFactory` probe. Descriptor records (sealed `NodeDescriptor`: `InterfaceNode` | `ClassNode` | `InlineNode`, `GraphRuleDescriptor`, `GraphInvariantDescriptor`, `PatternParameterDescriptor`) + Quarkus `@Recorder`. |
...
| `yaml/runtime/` | `casehub-desiredstate-yaml` | `io.casehub.desiredstate.yaml` | YAML surface — Jackson-based YAML→GraphDescriptor deserializer, `NodeSpecRegistry` (Jandex-discovered `@NodeTypeId` → NodeSpec class mapping), `VariableResolver` (Map → Preferences → Config fallthrough), `YamlGraphRecorder`, `DesiredStateModuleBridge` (`ModuleBridge<DesiredStateModuleContent>` for typed module expansion via yaml-core), `YamlNodeForEachAdapter`. YAML model types: `YamlGraph`, `YamlNode`, `YamlDesiredState`. |
| `yaml/deployment/` | `casehub-desiredstate-yaml-deployment` | `io.casehub.desiredstate.yaml.deployment` | Quarkus build extension: classpath YAML discovery at `META-INF/desiredstate/*.yaml`, `@NodeTypeId` registry scan, build-time validation (unknown types, dangling dependsOn, cycles, cross-surface namespace:name collisions via `DesiredStateGraphBuildItem`), module discovery via `YamlCoreJacksonModule` + `resolveExtensions()`, `GoalCompiler<Void>` bean registration. |
...
| `ts-dsl/runtime/` | `casehub-desiredstate-ts` | `io.casehub.desiredstate.ts` | TypeScript DSL surface — `TsGraphRecorder` (JSON envelope → GoalCompiler bridge). Single-graph and lifecycle compilers with carry-forward semantics. Envelope model records: `TsEnvelope`, `TsEnvelopeNode`, `TsLifecycleEnvelope`, `TsEnvelopePhase`. |
...
| `plugin/api/` | `casehub-desiredstate-plugin-api` | `io.casehub.desiredstate.plugin.api` | Step pipeline SPI — `StepPrimitive`, `StepResult`, `StepContext`, `StepParameters`, `YamlNodeSpec` (NodeSpec adapter for YAML-declared types), `PluginInterpolator`, `ExpressionEvaluator`. Pure Java. |
| `plugin/runtime/` | `casehub-desiredstate-plugin` | `io.casehub.desiredstate.plugin.runtime` | Plugin YAML model (`PluginModel`, `PluginParser`, `PluginStepDef`, `PluginHeader`, `PluginSpecSchema`, `PluginFieldDef`, `PluginProvisionerDef`, `PluginFaultPolicyDef`, `PluginCbrDef`, `PluginRasDef`), `StepPipelineExecutor`, `PrimitiveRegistry`, built-in Java primitives (`CompareStatePrimitive`, `AssertPrimitive`, `JsonExtractPrimitive`, `RestCallPrimitive`), `YamlPluginProvisioner`, `YamlPluginActualStateAdapter`, `YamlPluginCbrRegistrar`, `YamlPluginRasRegistrar`. Quarkus library. |
| `plugin/deployment/` | `casehub-desiredstate-plugin-deployment` | `io.casehub.desiredstate.plugin.deployment` | Quarkus build extension: plugin YAML discovery at `META-INF/desiredstate/plugins/*.yaml`, spec schema validation, primitive registry, interpolation reference validation, cross-file type coverage, compound primitive expansion. |
