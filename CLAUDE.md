...
| `plugin/runtime/` | `casehub-desiredstate-plugin` | `io.casehub.desiredstate.plugin.runtime` | Plugin YAML model (`PluginModel`, `PluginParser`, `PluginStepDef`, `PluginHeader`, `PluginSpecSchema`, `PluginFieldDef`, `PluginProvisionerDef`, `PluginFaultPolicyDef`, `PluginCbrDef`, `PluginRasDef`, `CompoundPrimitiveDef`), `StepPipelineExecutor`, `PrimitiveRegistry`, built-in Java primitives (`CompareStatePrimitive`, `AssertPrimitive`, `JsonExtractPrimitive`, `RestCallPrimitive`), `YamlPluginProvisioner`, `YamlPluginActualStateAdapter`, `PluginDescriptor`, `CompoundPrimitiveExpander`, `CbrPluginMetadata`, `YamlPluginRasRegistrar`. Quarkus library. |
